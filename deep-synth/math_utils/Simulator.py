...
Intersection = namedtuple(
    'Intersection', ['id', 'linkIndex', 'ray_fraction', 'position', 'normal'])
...
class Simulator:
    def __init__(self, mode='direct', bullet_server_binary=None, data_dir_base=None, verbose=False):
        ...
        if bullet_server_binary:
            ...
            self._bullet_server_binary = os.path.join(
                module_dir, '..', 'bullet_shared_memory_server')
    ...
    def connect(self):
        ...
        if self._bullet_server:
            print(
                f'Restarting by killing bullet server pid={self._bullet_server.pid}...')
            self._bullet_server.kill()
            # seems necessary to prevent deadlock on re-connection attempt
            time.sleep(1)
            self._bullet_server = None
        ...
        if self._mode == 'gui':
            ...
            # seems necessary to prevent deadlock on connection attempt
            time.sleep(1)
        ...
        if self._mode == 'gui':
            p.configureDebugVisualizer(
                p.COV_ENABLE_Y_AXIS_UP, 1, physicsClientId=self._pid)
            p.configureDebugVisualizer(
                p.COV_ENABLE_RGB_BUFFER_PREVIEW, 0, physicsClientId=self._pid)
            p.configureDebugVisualizer(
                p.COV_ENABLE_DEPTH_BUFFER_PREVIEW, 0, physicsClientId=self._pid)
            p.configureDebugVisualizer(
                p.COV_ENABLE_SEGMENTATION_MARK_PREVIEW, 0, physicsClientId=self._pid)
            # disable rendering during loading -> much faster
            p.configureDebugVisualizer(
                p.COV_ENABLE_RENDERING, 0, physicsClientId=self._pid)

    def __del__(self):
        if self._bullet_server:
            print(
                f'Process terminating. Killing bullet server pid={self._bullet_server.pid}...')
            self._bullet_server.kill()
    ...
    def run(self):
        if self._mode == 'gui':
            ...
            while True:
                ...
                if self._verbose:
                    print(
                        f'#contacts={len(contacts)}, contact_pairs={contacts.keys()}')
        else:
            self.step()
    ...
    def set_gui_rendering(self, enabled):
        ...
        for obj_id in self._obj_id_to_body.keys():
            ...
            # try to ignore room object "identity" transform
        ...
        p.configureDebugVisualizer(
            p.COV_ENABLE_RENDERING, 1 if enabled else 0, physicsClientId=self._pid)
        return enabled

    def add_mesh(self, obj_id, obj_file, transform, vis_mesh_file=None, static=False):
        if static:
            print('debug-----------------', p.GEOM_MESH, obj_file,
                  p.GEOM_FORCE_CONCAVE_TRIMESH, transform.scale, self._pid)
        ...
        if vis_mesh_file:
            vid = p.createVisualShape(p.GEOM_MESH, fileName=vis_mesh_file, meshScale=transform.scale,
                                      physicsClientId=self._pid)
        # w,x,y,z -> x,y,z,w (which pybullet expects)
        rot_q = np.roll(transform.rotation.elements, -1)
    ...
    def add_box(self, obj_id, half_extents, transform, static=False):
        cid = p.createCollisionShape(
            p.GEOM_BOX, halfExtents=half_extents, physicsClientId=self._pid)
        # w,x,y,z -> x,y,z,w (which pybullet expects)
        rot_q = np.roll(transform.rotation.elements, -1)
        mass = 0 if static else 1
        bid = p.createMultiBody(baseMass=mass,
    ...
    def add_object(self, node, create_vis_mesh=False, static=False):
        # TODO: need to otherwise account for mirror?
    ...
    def add_room(self, node, wall=True, floor=True, ceiling=False):
        ...
        body_ids = []
        if wall:
            # treat walls as room (=room.id, no suffix)
            body_wall = add_architecture(node, f'{basename}w.obj', '')
    ...
    def add_house(self, house, no_walls=False, no_ceil=False, no_floor=False, use_separate_walls=False, static=False):
        for node in house.nodes:
            ...
            if node.type == 'Room':
                ceil = False if no_ceil else not (
                    hasattr(node, 'hideCeiling') and node.hideCeiling == 1)
                wall = False if (no_walls or use_separate_walls) else not (
                    hasattr(node, 'hideWalls') and node.hideWalls == 1)
                floor = False if no_floor else not (
                    hasattr(node, 'hideFloor') and node.hideFloor == 1)
                node.body = self.add_room(
                    node, wall=wall, floor=floor, ceiling=ceil)
            if node.type == 'Box':
    ...
    def set_state(self, obj_id, position, rotation_q):
        body = self._obj_id_to_body[obj_id]
        # w,x,y,z -> x,y,z,w (which pybullet expects)
        rot_q = np.roll(rotation_q.elements, -1)
    ...
    def get_state(self, obj_id):
        ...
        pos, q = p.getBasePositionAndOrientation(
            bodyUniqueId=body.bid, physicsClientId=self._pid)
    ...
    def get_closest_point(self, obj_id_a, obj_id_b, max_distance=np.inf):
        ...
        cps = p.getClosestPoints(
            bodyA=bid_a, bodyB=bid_b, distance=max_distance, physicsClientId=self._pid)
        cp = None
        if len(cps) > 0:
    ...
    def get_contacts(self, obj_id_a=None, obj_id_b=None, only_closest_contact_per_pair=True,
        ...
        cs = p.getContactPoints(bodyA=bid_a, bodyB=bid_b,
                                physicsClientId=self._pid)
    ...
    def ray_test(self, from_pos, to_pos):
        hit = p.rayTest(rayFromPosition=from_pos,
                        rayToPosition=to_pos, physicsClientId=self._pid)
...
if __name__ == '__main__':
    ...
    # h = House(index=5)
    sim.add_house(h, no_walls=False, no_ceil=True,
                  use_separate_walls=False, static=False)
    sim.run()
